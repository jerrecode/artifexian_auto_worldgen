...
def test_atmogen_procedural_erosion_retains_condensate_bridge_and_exports():
    cfg = WorldConfig(seed=919191)
    cfg.resolution.width = 64
    cfg.resolution.height = 32
    cfg.resolution.history_myr = 50
    cfg.resolution.history_step_myr = 25
    ...
    cfg.tectonics.history_grid_height = 32
    cfg.tectonics.shape_control_points_per_subplate = 1
    cfg.climate.moisture_iterations = 4
    cfg.hydrology.surface_evolution_iterations = 1
    cfg.hydrology.sediment_routing_passes = 3
    cfg.simulation.earth_system_passes = 1
    cfg.simulation.final_climate_ocean_passes = 1
    ...
    cfg.astronomy.surface_volatiles = {"H2O": 0.01}
    cfg.astronomy.surface_condensible = "H2O"

    cfg.atmogen.enabled = True
    cfg.atmogen.fidelity = "FAST"
    cfg.atmogen.chemistry_mode = "fixed_species"
    cfg.atmogen.vertical_layers = 12
    cfg.atmogen.representative_columns_enabled = False
    cfg.atmogen.max_iterations = 30
    ...
    forcing = world["condensate_hydrology"]
    erosion_forcing = world["procedural_erosion_forcing"]

    assert forcing is not None
    assert world["coupling_summary"]["multicondensate_hydrology"]["enabled"] is True
    assert erosion_forcing.metadata["freeze_thaw_phase_source"] == (
    ...
    for key in (
        ...
        assert np.asarray(arrays[key]).shape == world["grid"].shape
