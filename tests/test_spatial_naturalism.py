...
def _azimuthal_boundary_variation(grid: SphereGrid, mask: np.ndarray, center: np.ndarray) -> float:
    ...
    a = np.cross(ref, c); a /= np.linalg.norm(a)
    b = np.cross(c, a); b /= np.linalg.norm(b)
    ...
    for lo, hi in zip(bins[:-1], bins[1:]):
        r = radius[(az >= lo) & (az < hi)]
        if r.size:
            outer.append(float(np.max(r)))
    outer = np.asarray(outer)
    return float(np.std(outer) / max(np.mean(outer), 1e-12))


def test_hotspot_province_is_not_a_circular_gaussian_disc() -> None:
    grid = SphereGrid(240, 120)
    center = np.array([1.0, 0.0, 0.0])
    field = irregular_blob_field(
        grid,
        np.asarray([center]),
        np.asarray([7.0]),
        np.asarray([1.0]),
...
def test_irregular_geological_near_mask_has_rough_non_radial_margin() -> None:
    grid = SphereGrid(240, 120)
    source = np.zeros(grid.shape, dtype=bool)
    source[grid.height // 2, grid.width // 2] = True
    near = irregular_near(source, grid, 1200.0)
...
def test_tiny_lake_is_not_promoted_to_planetary_marine_reservoir() -> None:
...
def test_large_ocean_is_retained_as_marine_thermal_source() -> None:
    grid = SphereGrid(160, 80)
    water = np.zeros(grid.shape, dtype=bool)
...
def test_runtime_installer_replaces_blob_and_resource_radius_primitives() -> None:
    import worldgen.resources as resources
    import worldgen.tectonics as tectonics

    assert tectonics._blob_field is irregular_blob_field
    assert resources._near is irregular_near
