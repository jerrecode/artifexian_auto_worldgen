...
def _land_components_wrap(grid: SphereGrid, land: np.ndarray) -> np.ndarray:
    """Return canonical spherical 8-connected land-component labels.

    The name is retained for private-call compatibility, but the implementation is
    no longer a seam-only special case: longitude wrapping and antipodal pole
    crossing are both delegated to the same topology layer used by the physical
    simulation.
    """
...
def _causal_settlement_sites(
    ...
    if len(candidates) <= count:
        selected = candidates
        comp = _land_components_wrap(grid, terrain.land)
    ...
    n=len(candidates)
    comp_map=_land_components_wrap(grid, terrain.land)
