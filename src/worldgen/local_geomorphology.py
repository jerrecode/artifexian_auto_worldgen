...
class LocalGeomorphologySolver:
    ...
    def solve(self, key: TileKey) -> LocalGeomorphologyResult:
        ...
        # Apply the general finite-domain anchoring once, then add the already
        # edge-tapered procedural perturbation. Keeping the procedural term outside
        # the second taper makes the exported field identical to the perturbation
        # actually applied to terrain and preserves its area-weighted zero mean.
        ...
        metadata = {
            ...
            "procedural_area_weighted_mean_m": (
                ...
                if np.sum(area_m2) > 0.0
                else 0.0
            ),
            "procedural_semantics": "zero-area-mean unresolved morphology applied once after finite-domain anchoring; excluded from the physical sediment mass ledger",
