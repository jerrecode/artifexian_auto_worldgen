...
def _clone_with_globals(fn: Callable, replacements: dict[str, object]) -> Callable:
    globs = dict(fn.__globals__)
    globs.update(replacements)
    clone = types.FunctionType(
        fn.__code__, globs, name=fn.__name__, argdefs=fn.__defaults__, closure=fn.__closure__
    )
    clone.__kwdefaults__ = fn.__kwdefaults__
    clone.__annotations__ = dict(getattr(fn, "__annotations__", {}))
    clone.__doc__ = fn.__doc__
    return clone


def install_spatial_naturalism_fix() -> None:
    """Install deterministic natural-shape replacements before pipeline binding."""
    from . import climate as climate_mod
    from . import resources as resources_mod
    from . import tectonics as tectonics_mod

    if getattr(tectonics_mod, "_spatial_naturalism_installed", False):
        return

    original_generate_tectonics = tectonics_mod.generate_tectonics
    original_build_climate = climate_mod.build_climate
    original_build_resources = resources_mod.build_resources
    original_climate_distance = climate_mod.distance_to

    tectonics_mod._blob_field = irregular_blob_field
    resources_mod._near = irregular_near

    @functools.wraps(original_generate_tectonics)
    def generate_tectonics_natural(*args, **kwargs):
        result = original_generate_tectonics(*args, **kwargs)
        result.metadata["hotspot_lip_footprint_model"] = (
            "anisotropic lobate spherical provinces with deterministic multi-scale rough margins"
        )
        return result

    @functools.wraps(original_build_resources)
    def build_resources_natural(*args, **kwargs):
        result = original_build_resources(*args, **kwargs)
        result.metadata["proximity_envelope_model"] = (
            "heterogeneous geodesic reach; no exact-radius geological discs"
        )
        return result

    @functools.wraps(original_build_climate)
    def build_climate_natural(
        grid,
        astronomy,
        terrain,
        ocean,
        ...
        texture = None if static_noise is None else getattr(static_noise, "climate_texture", None)

        def scoped_distance(mask, candidate_grid):
            if candidate_grid is grid and mask is terrain.ocean:
                return marine_thermal_distance(
                    grid,
                    terrain.ocean,
                    ocean.elevation_km,
                    inland_scale_km=float(cfg.inland_thermal_length_km),
                    climate_texture=texture,
                )
            return original_climate_distance(mask, candidate_grid)

        cloned = _clone_with_globals(original_build_climate, {"distance_to": scoped_distance})
        result = cloned(
            grid,
            astronomy,
            terrain,
        ...
        result.metadata["marine_thermal_distance_model"] = (
            "large-water-component geodesic fetch with circulation, relief and climate-texture anisotropy"
        )
        result.metadata["tiny_water_continentality_halos_suppressed"] = True
        return result

    tectonics_mod.generate_tectonics = generate_tectonics_natural
    climate_mod.build_climate = build_climate_natural
    resources_mod.build_resources = build_resources_natural
    tectonics_mod._spatial_naturalism_installed = True


__all__ = [
    "install_spatial_naturalism_fix",
