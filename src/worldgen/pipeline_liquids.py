...
class WorldPipeline(_AdaptiveWorldPipeline):
    ...
    def _solve_liquid_state(
        ...
        *,
        ...
        return self._stage(
            stage_name,
            lambda: place_partitioned_liquids(
                grid,
                bed_datum_km,
                inventories,
                atmogen_result.surface,
                temperature_k=float(atmogen_result.atmosphere.temperature_k[0]),
            )
            if atmogen_result is not None
            else solve_surface_liquids(
                grid,
                bed_datum_km,
